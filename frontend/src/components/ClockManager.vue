...
const handleClockCreation = async () => {
    try {
        const newClock = await createClock(
            userId.value,
            {
...
const message = computed(() => {
    if (clockedIn.value) {
        return `You are currently clocked in since ${formatDate(latestClock.value.time.toString())}`;
    } else {
...
<template>
    <div class="flex flex-col bg-gray-900 text-gray-100 items-center justify-center">
        <main class="w-full flex flex-col items-center py-4">
            <button v-on:click="handleClockCreation"
                :class="['text-2xl', 'font-bold', 'text-white', 'py-2', 'px-4', 'rounded-full', 'shadow-lg', clockedIn ? 'bg-red-500' : 'bg-green-500']">
