<template>
  <div class="mx-auto mt-10 p-6 bg-gray-900 rounded-lg shadow-xl">
    ...
    <p
      ...
      class="text-white text-center my-2 -mt-5 font-bold"
    >
      or
    </p>
    ...
    <div v-if="isCreating">
      <h2 class="text-2xl font-bold mb-6 text-center text-white mt-2">
        Create an account 👋🏻
      </h2>
      <input
        v-model="newEmail"
...
export default {
  ...
  methods: {
    async getUser() {
      try {
        const response = await fetch(
          `https://` +
            import.meta.env.VITE_BACKEND_DNS +
            `/api/users?email=${this.email}&username=${this.username}`
        );
    ...
    async createAccount() {
      try {
        const response = await fetch(
          "https://" + import.meta.env.VITE_BACKEND_DNS + "/api/users",
          {
            method: "POST",
