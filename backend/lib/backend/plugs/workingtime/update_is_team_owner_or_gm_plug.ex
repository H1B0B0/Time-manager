defmodule Backend.Plugs.Workingtime.UpdateIsTeamOwnerOrGMPlug do
  import Plug.Conn
  ...
  def call(conn, _opts) do
    ...
    if auth_user.role_id in [RolesEnum.role_general_manager, RolesEnum.role_manager] do
      ...
      working_time_id = conn.params["working_time_id"] || conn.params["id"]
      ...
      if user_id == auth_user.id do
        Logger.info("User is updating their own working time, access granted.")
        conn
      else
        ...
        user_teams = TeamsUsers.get_teams_for_user(user)
